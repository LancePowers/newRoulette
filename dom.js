...
document.getElementById('spin').addEventListener('click',function(){spinAction();});
document.getElementById('reset').addEventListener('click',function(){spinAction();});
...
function assignDegrees() {
  ...
  for (var i = 0; i < wheelOrder.length; i++) {
    var test = "";
    wheelPosition.push({order: wheelOrder[i],increment: localIncrement});
